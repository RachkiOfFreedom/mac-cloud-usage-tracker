...
describe("normalizeUsageRecord", () => {
  ...
  it("rejects invalid timezone offsets", () => {
    const invalidOffsets = ["+25:00", "-25:00", "+99:99", "-13:00", "-14:00", "+15:00"];
  ...
  it("accepts boundary timezone offsets", () => {
    // +14:00 is the maximum valid positive offset (Line Islands)
    const plus14 = normalizeUsageRecord({
      accountId: "acct",
      instanceFamily: "mac1.metal",
      region: "us-west-2",
      usageMinutes: 1,
      startIso: "2025-01-15T00:00:00+14:00"
    });
    expect(plus14.startIso).toBe("2025-01-14T10:00:00.000Z");

    // -12:00 is the minimum valid negative offset
    const minus12 = normalizeUsageRecord({
      accountId: "acct",
      instanceFamily: "mac1.metal",
      region: "us-west-2",
      usageMinutes: 1,
      startIso: "2025-01-15T00:00:00-12:00"
    });
    expect(minus12.startIso).toBe("2025-01-15T12:00:00.000Z");
  });
});
